...
function App(){
 ...
 {loading&&<div className="empty">Loading current stock…</div>}{error&&<div className="error">Unable to load stock: {error}</div>}
 {!loading&&!error&&<div className="grid">{list.map(p=><article className="card" key={p.id}><div className="image">{p.image_url?<img src={p.image_url} alt={`${p.brand} ${p.model}`}/>:<div className="device"><span>{(p.brand||"M").slice(0,2)}</span></div>}<span className={p.stock?"dot availableDot":"dot outDot"}>{p.stock?"Available":"Out of stock"}</span></div><div className="body"><div className="brand">{p.brand}</div><h3>{p.model}</h3><p>{[p.ram,p.storage].filter(Boolean).join(" • ")}</p>{p.color&&<p>{p.color}</p>}<div className="price">{money(p.price)}</div><button disabled={!p.stock} onClick={()=>ask(p)}>{p.stock?"💬 Enquire on WhatsApp":"Currently unavailable"}</button></div></article>)}</div>}
 {!loading&&!error&&!list.length&&<div className="empty">No matching phones found.</div>}</main>
 <section className="contact" id="contact"><div><div className="eyebrow">VISIT OUR SHOP</div><h2>Bhanu Mobile Plaza</h2><p>📍 Seethangaram</p><p>📞 81255 52523</p></div><div className="contactBtns"><a href={`https://wa.me/${PHONE}`} target="_blank">WhatsApp us</a><a href="https://www.google.com/maps/search/?api=1&query=Bhanu+Mobile+Plaza+Seethangaram" target="_blank">Open Maps ↗</a></div></section><footer><b>Bhanu Mobile Plaza</b><span>Seethangaram · 81255 52523</span><small>© 2026 Bhanu Mobile Plaza</small></footer></div>
}
